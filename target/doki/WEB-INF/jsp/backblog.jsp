...
<div class="navbar navbar-fixed-top navbar-inverse" role="navigation">
  ...
  <div id="wrapper">
				...
				<div class="modal-body">
					<form class="form-horizontal" id="updateList">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
							<input type="text" disabled="disabled" class="form-control" id="up_blogwatch" name="up_blogwatch" >	
							</div>
						</div>
						<div class="form-group">
	...
	<div class="row">
		...
		<div class="col-md-4">
			<form class="form-inline" action="<%=path%>/back/blogsearch">
				<div class="form-group">
	...
	<div class="col-md-2"> </div>
		<div class="col-md-2">
			<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#insmodal">添加博客</button>
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title" id="myModalLabel">新建博客</h4>
						</div>
						<form class="form-horizontal" id="saveList" action="<%=path%>/back/insert" method="post" enctype="multipart/form-data">
						<div class="modal-body">
							
								<div class="form-group">
									<label class="col-sm-2 control-label">博客标题</label>
									<div class="col-sm-10">
										<input type="text" class="form-control" id="blogtitle" name="blogtitle" placeholder="请输入标题">
									</div>
								</div>
								<div class="form-group">
									<label for="password" class="col-sm-2 control-label">博客内容</label>
									<div class="col-sm-10">
										<textarea class="form-control" rows="6" id="blogcontent" name="blogcontent"></textarea>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										...
										<input type="text" id="blogtypename" name="blogtypename" placeholder="输入博客分类" class="form-control border-0 shadow form-control-lg" list="typelist">
			...
			<table class="table">
				...
				<c:forEach items="${bloglist.list}" var="blog">
					<tr>
						...
						<td width="20%">${blog.blogtitle }</td>
						<td width="30%">${blog.blogcontent }</td>
						<td width="10%">${blog.blogtypeid }</td>
						<td width="10%">${blog.blogwatch }</td>
						<td width="10%">${blog.username }</td>
						<td width="10%">
						...
						<c:if test="${blog.delFlag==1 }">
						<button type="button" class="btn btn-danger del_btn" name="${blog.id }" id="del_btn" data-toggle="modal" data-target="#deleteModal">删除</button>
						</c:if>
						<c:if test="${blog.delFlag==0 }">
						<button type="button" class="btn btn-success res_btn" name="${blog.id }" id="res_btn" data-toggle="modal" data-target="#resModal">恢复</button>
						</c:if>
	...
	<div class="row">
		<div class="col-md-4">当前页数:${bloglist.pageNum}页,总页数:${bloglist.pages}页，总条数:${bloglist.total}条</div>
		<div class="col-md-4">
			<nav aria-label="Page navigation">
				<ul class="pagination">
					...
					<c:forEach items="${bloglist.navigatepageNums}" var="pNum">
						...
						<c:if test="${pNum!=bloglist.pageNum}">
							<li><a id="navpage" name="${pNum }" href="<%=path%>/back/blogsearch?blogtitle=&pn=${pNum }">${pNum}</a></li>
						</c:if>
					</c:forEach>
	...
	$(function(){
		...
		var pnum='${bloglist.pageNum}';
		var prenum='${bloglist.pageNum-1}';
		var nexnum='${bloglist.pageNum+1}';
		var pn='${bloglist.pages}';
		var gettitle=$("#search_blogtitle").val();
		var indexpn="<%=path%>/back/blogsearch?blogtitle="+gettitle+"&pn=1";
		var prepn="<%=path%>/back/blogsearch?blogtitle="+gettitle+"&pn="+prenum;
		var nextpn="<%=path%>/back/blogsearch?blogtitle="+gettitle+"&pn="+nexnum;
		
		
	<%-- 	var navpn="<%=path%>/back/blogsearch?blogtitle="+gettitle+"&pn="+navpage; --%>
		
		var endpn="<%=path%>/back/blogsearch?blogtitle="+gettitle+"&pn="+pn;
...
//新建模态框保存按钮
		...
		$(document).on("click", ".edit_btn", function() {
			...
			var blogwatch1=$(this).parent().parent().children("td").eq(4).text();
			var bloguploader=$(this).parent().parent().children("td").eq(5).text();
