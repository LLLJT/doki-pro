       ...
       <div class="col-md-2"></div>
        <div class="col-md-8 mx-auto">
          <div class="site-heading">
            ...
            <h3>  <button type="button" class="btn btn-primary btn-lg btn-sub" name="${userid }">${ifwatch }</button></h3>
            
            
          </div>
        </div>
         <div class="col-md-2"><input hidden="true" id="getid" value="${userid }"></div>
      </div>
    </div>
  </header>
...
<script src="<%=path%>/js/cookie/getcookie.js"></script>
	...
	<script type="text/javascript">
		...
		$("#home_btn").click(function(){
			...
			window.location.href="<%=path%>/center/tocenter";
		...
		$("#modifylist_btn").click(function(){
			var id=parseInt('${userid}');
			var cookiestr = getCookie("user");
			var strid=parseInt(cookiestr.split("#")[2]);
			if(id!=strid){
				alert("您不能修改他人信息");
				return false;
		...
		$(".btn-sub").click(function(){
			...
			if(buttonValue=='已关注'){
				...
				//取消关注成功
				alert("取消关注成功");
				window.location.href="<%=path%>/center/deletewatch2?userid1="+userid1+"&userid2="+userid2;
				$(this).html('未关注');
			}
			else{
				alert("关注成功");
				window.location.href="<%=path%>/center/insertwatch2?userid1="+userid1+"&userid2="+userid2;
				$(this).html('已关注');
			}
